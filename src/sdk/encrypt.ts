...
export const encrypt8 = (value: number, publicKey: TfheCompactPublicKey): Uint8Array => {
  const uint8Array = new Uint8Array([value]);
  const encrypted = CompactFheUint8List.encrypt_with_compact_public_key(uint8Array, publicKey);
  return encrypted.serialize();
};

export const encrypt16 = (value: number, publicKey: TfheCompactPublicKey): Uint8Array => {
